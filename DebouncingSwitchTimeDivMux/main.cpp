...
// global constants
const TickType TickLength = 1000;     // 1/1000 second
const TickType DebounceDelay = 50000; // 1/1000 second

// global variables
unsigned count = 0; // count of button presses
uint8_t state = 0;    // state of button press
...
void setup()
{
    ...
    DDRD = 0b11111111; // set PORTD[0:7] as output

    // set PORTB[0:1] as input
...
void loop()
{
    ...
    for (int i = 0; i < SwitchControl::NumSwitches; i++)
    {
        if (SwitchControl::switches[i].debounce())
        {
            state = 1;
            currentPin = i;
        }
    ...
    count %= 100; // roll over count to 0 if count > 99
